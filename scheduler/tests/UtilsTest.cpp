...
BOOST_AUTO_TEST_CASE(parse_valid_offset_basic) {
    // 2023-10-26T14:00:00+0200 -> 12:00:00 UTC
    const std::string ts = "2023-10-26T14:00:00+0200";
    auto res = parseIso8601(ts);
    BOOST_CHECK(res.has_value());

    auto str = toIso8601(res.value());
    // toIso8601 always outputs Z suffix (UTC)
    BOOST_CHECK_EQUAL(str, "2023-10-26T12:00:00Z");
}

BOOST_AUTO_TEST_CASE(parse_valid_offset_extended) {
    // 2023-10-26T14:00:00+02:00 -> 12:00:00 UTC
    const std::string ts = "2023-10-26T14:00:00+02:00";
    auto res = parseIso8601(ts);
    BOOST_CHECK(res.has_value());
...
BOOST_AUTO_TEST_CASE(parse_no_offset_assumes_utc) {
    // 2023-10-26T12:00:00 -> 12:00:00 UTC
    const std::string ts = "2023-10-26T12:00:00";
    auto res = parseIso8601(ts);
    BOOST_CHECK(res.has_value());
...
BOOST_AUTO_TEST_CASE(parse_decimal_seconds) {
    const std::string ts = "2023-10-26T12:00:00.1564Z";
    auto res = parseIso8601(ts);
    BOOST_CHECK(res.has_value());
...
BOOST_AUTO_TEST_CASE(parse_invalid_date) {
    const std::string ts = "2023-13-45T12:00:00Z";
    auto res = parseIso8601(ts);
