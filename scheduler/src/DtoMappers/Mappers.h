#ifndef SCHEDULER_MAPPERS_HPP
#define SCHEDULER_MAPPERS_HPP
#pragma once
...
auto f_toTrivialDto(const scheduler::ScheduleImpact &, int impactId)
    -> TrivialImpactModel;
auto f_toTrivialDto(const scheduler::InternalBlock &, int trivialImpactId)
    -> TrivialJobModel;
auto f_fromDto(const TrivialImpactModel &) -> scheduler::ScheduleImpact;
auto f_fromDto(const TrivialJobModel &, int impactId)
...
struct FromDtoFn {
    ...
    static auto withImpactId(int impactId) {
        return [impactId](
                   const TrivialJobModel &block) -> scheduler::ScheduleBlock {
...
struct ToDtoFn {
    ...
    static auto withImpactId(int impactId) {
        return [impactId](const scheduler::InternalBlock &block) -> JobModel {
...
struct ToTrivialDtoFn {
    static auto withImpactId(int impactId) {
        return
            [impactId](
                const scheduler::ScheduleImpact &impact) -> TrivialImpactModel {
                return f_toTrivialDto(impact, impactId);
            };
    }

    static auto withTrivialImpactId(int trivialImpactId) {
        return [trivialImpactId](
                   const scheduler::InternalBlock &block) -> TrivialJobModel {
...
concept MappableModel =
    ...
    std::same_as<T, TrivialJobModel> || std::same_as<T, TrivialImpactModel>;
...
inline auto fromTrivialDtoAll(const std::vector<TrivialJobModel> &models,
                              int impactId) {
    return models | std::views::transform(fromDto.withImpactId(impactId)) |
           std::ranges::to<std::vector>();
}

} // namespace scheduler::mappers

#endif // SCHEDULER_MAPPERS_HPP
