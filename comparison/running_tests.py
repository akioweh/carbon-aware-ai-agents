...
def calc_length_of_computation(
    workload: float, fullPower: float, power_mode: float
) -> float:
    return workload / (fullPower * power_mode)
...
def run_single_test(
    ...
    for workload in get_workload_amounts():
        for power_mode in power_coeficients():
            ...
            length_of_computation = calc_length_of_computation(
                workload, full_power, power_mode
            )
            sdkCarbonEmissions = (
                workload
