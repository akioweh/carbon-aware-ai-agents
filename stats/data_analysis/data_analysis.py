...
def week_analysis(df: pd.DataFrame):
    ...
    for region in REGIONS:
        ...
        pivot = rd.pivot_table(index='slot', columns=['iso_year', 'iso_week'], values='actual', aggfunc='mean')
...
def generate_report(df: pd.DataFrame, desc_stats: pd.DataFrame, week_data: dict, volatile_slots: list):
    ...
    for region in REGIONS:
        ...
        for ws in data['week_summaries']:
            lines.append(f'| {ws["year"]} | {ws["week"]} | {ws["mean"]:.1f} | {ws["std"]:.1f} | {ws["slots"]} |')
        lines.append('')

        # Overall slot variability
        slot_std = data['slot_std']
        if len(slot_std) > 0:
            lines.append(f'- Average across-week slot SD: **{slot_std.mean():.1f}** gCO₂/kWh')
            lines.append(f'- Max across-week slot SD: **{slot_std.max():.1f}** gCO₂/kWh (slot {slot_std.idxmax()})\n')
    ...
    lines.append(f'- **Highest variability**: {region_cv.idxmax()} (CV = {region_cv.max():.3f})')
    lines.append(f'- **Lowest variability**: {region_cv.idxmin()} (CV = {region_cv.min():.3f})')

    # Peak CI
    for region in REGIONS:
        rd = df[df['region'] == region]
        peak = rd.loc[rd['actual'].idxmax()]
        lines.append(f'- **{region}** peak: {peak["actual"]:.0f} gCO₂/kWh at {peak["ts"]}')
    lines.append('')
    ...
    lines.append('Based on the analysis above, the following preprocessing strategies may improve model training:\n')
    lines.append(
