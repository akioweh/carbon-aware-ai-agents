...
for region in sorted(regions):
    ...
    results_notrend[region] = {
        'train': train, 'test': test,
        'preds': preds_nt, 'mae': mae_nt, 'rmse': rmse_nt,
    }
...
for ax, region in zip(axes, sorted(regions)):
    r = best[region]
    # Training data (faded)
    ax.plot(r['train']['ts'], r['train']['greenness'],
            linewidth=0.6, color='#90CAF9', alpha=0.5, label='Train (actual)')
    # Split line
    ax.axvline(split_date, color='gray', linestyle='--', linewidth=1, label='Train/Test split')
    # Test actual
    ax.plot(r['test']['ts'], r['test']['greenness'],
            linewidth=1, color='#2196F3', label='Test (actual)')
    # Predictions
    ax.plot(r['test']['ts'], r['preds'],
            linewidth=1.2, color='#F44336', linestyle='--', label='Predictions')
