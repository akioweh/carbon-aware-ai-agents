...
class TestHistoricalData:
    ...
    def test_filter_start_only(self, tmp_db):
        ...
        db_utils.insert_historical_data_bulk(data)
        rows = db_utils.get_historical_data('Data-Center-1', start_time=base + timedelta(hours=20))
    ...
    def test_filter_end_only(self, tmp_db):
        ...
        db_utils.insert_historical_data_bulk(data)
        rows = db_utils.get_historical_data('Data-Center-1', end_time=base + timedelta(hours=3))
...
class TestHistoricalCache:
    def test_save_and_get(self, tmp_db):
        ...
        cached = db_utils.get_historical_cache('Data-Center-1', now - timedelta(hours=1), now)
    ...
    def test_replaces_on_save(self, tmp_db):
        now = datetime.now(timezone.utc).replace(second=0, microsecond=0)
        entries1 = [{'timestamp': now, 'load': 1.0, 'carbon_intensity': 100.0}]
        entries2 = [{'timestamp': now, 'load': 2.0, 'carbon_intensity': 200.0}]
        db_utils.save_historical_cache('Data-Center-1', entries1)
        db_utils.save_historical_cache('Data-Center-1', entries2)

        cached = db_utils.get_historical_cache('Data-Center-1', now - timedelta(hours=1), now + timedelta(hours=1))
        assert len(cached) == 1
...
class TestCarbonSync:
    def test_has_carbon_data_false_when_missing(self, tmp_db, monkeypatch):
        ...
        monkeypatch.setattr('db_utils.CARBON_DB_FILE', Path('/nonexistent/carbon.db'))
